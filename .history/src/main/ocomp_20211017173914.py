    ...
    async def on_message(self, message):
        ...
        if tokens[0].startswith('.addme'):
            ...
            if nonactivity == 0:
                ...
                'the battle_id you added entered could not be found'.format(message))
                return
            elif nonactivity == 1:
                await message.channel.send('{0.author.mention}, '
                'the battle_id you added entered is set to private'.format(message))
                return
            elif nonactivity == 2:
                await message.channel.send('{0.author.mention}, '
                'your battle_id was succesfully added to my memory!'.format(message))
                return
