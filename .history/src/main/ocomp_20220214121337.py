...
@bot.command(name='prefix', help='[Restricted to admins] changes my prefix to whatever you want!')
@has_permissions(administrator=True)
async def changeprefix(ctx, new_prefix):
    """Changes the bot prefix to the new_prefix"""

    await replace_prefix(str(ctx.guild.id), new_prefix)
    await ctx.send(f'{ctx.message.author.mention}, '
                   f'the prefix has been changed to {new_prefix}')
...
async def changeprefix_error(ctx, error):
    """Error handling for lacking admin permissions
    when running changeprefix
    """
    if isinstance(error, CheckFailure):
        msg = f'{ctx.message.author.mention}, only admins can change the prefix!'
        await ctx.send(msg)


@changeprefix_slash.error
async def changeprefix_slash_error(ctx, error):
    """Error handling for lacking admin permissions
    when running changeprefix_slash
    """
    if isinstance(error, CheckFailure):
        msg = f'{ctx.author.mention}, only admins can change the prefix!'
        await ctx.send(msg)
...
async def addme_slash(ctx, battle_id):
    ...
    nonactivity = await set_user(str(ctx.guild.id), battle_id,
                                 replace_last_char(str(ctx.author), '#', '-'))
    if nonactivity == 0:
        await ctx.send('{0.author.mention}, '
                       'the battle_id you added entered '
                       'could not be found'.format(ctx))
        return
    if nonactivity == 1:
        await ctx.send('{0.author.mention}, '
                       'the battle_id you added entered '
                       'is set to private'.format(ctx))
        return
    if nonactivity == 2:
        await ctx.send('{0.author.mention}, '
                       'your battle_id was succesfully '
                       'added to my memory!'.format(ctx))
...
async def compare(ctx, *args):
    ...
    try:
        hero = args[2].lower()
        data = await database_compare(str(ctx.guild.id), args[0], args[1], hero)
        player_1_hashtag = replace_last_char(await user_top_match(str(ctx.guild.id),
                                                                  args[0]), '-', '#')
        player_2_hashtag = replace_last_char(await user_top_match(str(ctx.guild.id),
                                                                  args[1]), '-', '#')
...
async def compare_slash(ctx, user_1, user_2, hero_name):
    ...
    try:
        ...
        data = await database_compare(str(ctx.guild.id), user_1, user_2, hero)
        player_1_hashtag = replace_last_char(await user_top_match(str(ctx.guild.id),
                                                                  user_1), '-', '#')
        player_2_hashtag = replace_last_char(await user_top_match(str(ctx.guild.id),
                                                                  user_2), '-', '#')
...
async def single_hero_stats(ctx, hero_name):
    ...
    try:
        ...
        if err == 'no playtime':
            await ctx.send(f'{ctx.message.author.mention}, '
                           'you have no playtime on {hero_name} this season!')
            return
...
async def single_hero_stats_slash(ctx, hero_name):
    ...
    try:
        data = await find_hero(str(ctx.guild.id), replace_last_char(
            str(ctx.author), '#', '-'), hero_name.lower())
        await ctx.send(f'{ctx.author.mention}, '
                       f'here are your stats on {hero_word_match(hero_name)} per 10: {data}')
        return
    except ValueError as err:
...
bot.run('ODk3OTA5MTcwODA2ODgyMzY1.YWchFA.JZ2kTI5sK8BkfoU6JPnuX19oW0Q')
