...
hero_list = ['ana', 'ashe', 'baptiste', 'bastion', 'brigitte', 'cassidy', 'dva', 'doomfist',
             'echo', 'genji', 'hanzo', 'junkrat', 'lucio', 'mei', 'mercy', 'moira',
...
cred = credentials.Certificate(
    "overbuff-compare-firebase-adminsdk-h8051-7f0b808aa7.json")
...
async def set_user(guild_id, battle_id, nickname):
    ...
    if nonactivity == 1:
        ...
        return 1
    print("active")
    ref.child(guild_id).child('Users').update(
        {nickname: {'battle_id': battle_id}})
    return 2


async def user_match(guild_id, nickname):
    """matches an abbreviated nickname to a list of users in the database"""
    snapshot = ref.child(guild_id).child('Users').get()
    users_list = []
...
async def user_top_match(guild_id, nickname):
    """matches an abbreviated nickname to a list of users in the database,
    but if no user is found, searches within the top users list as well."""
    try:
        return await user_match(guild_id, nickname)
    except (ValueError, AttributeError):
        snapshot = db.reference('TopUsers').get()
        users_list = []
        for key, _ in snapshot.items():
            users_list.append(key)
        return word_match(nickname, users_list)
...
async def find_hero_data(guild_id, nickname, hero, include_top=False):
    """Looks for a hero in the given player's stats page."""
    battle_id = ''
    snapshot = ref.child(guild_id).child('Users').get()

    if include_top:
        snapshot.update(db.reference('TopUsers').get())
        real_name = await user_top_match(guild_id, nickname)
    else:
        real_name = await user_match(guild_id, nickname)
    ...
    try:
        ...
        raise AttributeError('no playtime') from err


async def find_hero(guild_id, nickname, hero):
    """Returns data (in a string) for a player's stats on a specific hero"""
    results = await find_hero_data(guild_id, nickname, hero)
    response = ''
...
async def refresh_top100():
    ...
    player_response_list = soup.find(
        class_='table-condensed-mobile').contents[1]

    for player in player_response_list:
        player_url = player.contents[0].contents[0]['href']
        player_id = player_url[12:]
        top_ref = db.reference('/TopUsers')
        top_ref.update({player_id: {'battle_id': player_id}})


async def database_compare(guild_id, player_1, player_2, hero):
    """Compares 2 players's overbuff stats on a particular hero"""
    response = ''

    p1_data = await find_hero_data(guild_id, player_1, hero, include_top=True)
    p2_data = await find_hero_data(guild_id, player_2, hero, include_top=True)

    spaces = ' ' * 15

    try:
        for key, val in dict_zip(p1_data, p2_data).items():
            val_1 = val[0]
            val_2 = val[1]
            space_gap = ' ' * \
    ...
    if length_possibles != 1:
        for item in possibles:
            if word == item.lower() or word == item[:item.find('-')].lower():
                return item
        raise ValueError(
            f'{word} was not found in my database')
    return possibles[0]


def hero_word_match(hero):
    """Runs word_match for a hero onto the
    hero hero_list and parses it
    """
    try:
        lowercase_hero = word_match(hero, hero_list)
    except ValueError as err:
        if err == f'{hero} was not found in my database' as err:
            raise AttributeError('this hero does not exist') from err
    if lowercase_hero == 'wreckingball':
        return 'Wrecking Ball'
    elif lowercase_hero == 'soldier76':
        return 'Soldier: 76'
    elif lowercase_hero == 'dva':
        return 'D.Va'
    else:
        return lowercase_hero.capitalize()


async def search_prefix(guild_id):
    """Returns the prefix for the current guild from the database"""
    return ref.child(guild_id).child('Prefix').get()


async def replace_prefix(guild_id, new_prefix):
    """Updates the prefix for the current guild from the database
    with the new prefix
    """
    ref.child(guild_id).child('Prefix').set(new_prefix)


async def leave_guild(guild_id):
    """Removes all guild data from the database"""
    ref.child(guild_id).delete()
