...
class Database():
    ...
    """
    async def find_hero_data(self, nickname):
        battle_id = ''
        snapshot = ref.get()
        ...
        return await crawl_search_hero(battle_id, word_match(hero))

    async def find_hero(self, nickname, hero):
        results = await Database.find_hero_data(self, nickname)
        response = ''
    ...
    async def compare(self, p1, p2, hero):
        ...
        try:
            ...
            raise ValueError('one of these players has not played this hero this season!')
