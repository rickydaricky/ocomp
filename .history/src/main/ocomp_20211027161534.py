...
from database import Database, replace_last_char, search_prefix, \
    replace_prefix, leave_guild, hero_word_match, user_match
...
async def addme_slash(ctx, battle_id):
    ...
    nonactivity = await db.set_user(str(ctx.guild.id), battle_id,
                                    replace_last_char(str(ctx.message.author), '#', '-'))
    if nonactivity == 0:
        await ctx.send('{0.author.mention}, '
                       'the battle_id you added entered '
...
async def compare(ctx, *args):
    ...
    try:
        ...
        player_1_hashtag = replace_last_char(await user_match(
            args[0]), '-', '#')
        player_2_hashtag = replace_last_char(await user_match(
            args[1]), '-', '#')
...
async def compare_slash(ctx, user_1, user_2, hero_name):
    ...
    try:
        ...
        data = await db.compare(str(ctx.guild.id), user_1, user_2, hero)
        player_1_hashtag = replace_last_char(await user_match(
            user_1), '-', '#')
        player_2_hashtag = replace_last_char(await user_match(
            user_2), '-', '#')
