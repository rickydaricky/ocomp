...
async def set_user(self, guild_id, battle_id, nickname):
...
async def user_match(self, guild_id, nickname):
...
async def user_top_match(self, guild_id, nickname):
    """matches an abbreviated nickname to a list of users in the database,
    but if no user is found, searches within the top users list as well."""
    try:
        return user_match(self, guild_id, nickname)
    except ValueError:
        return 

async def find_hero_data(self, guild_id, nickname, hero):
    """Looks for a hero in the given player's stats page."""
    battle_id = ''
    snapshot = ref.child(guild_id).child('Users').get()
    real_name = await Database.user_match(self, guild_id, nickname)
...
async def find_hero(self, guild_id, nickname, hero):
    """Returns data (in a string) for a player's stats on a specific hero"""
    results = await Database.find_hero_data(self, guild_id, nickname, hero)
    response = ''
