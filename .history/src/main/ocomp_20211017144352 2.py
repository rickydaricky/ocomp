import discord
from database import Database, replace_hashtag, user_match
    ...
    async def on_message(self, message):
        ...
        if tokens[0].startswith('.compare'):
            ...
            try:
                ...
                await message.channel.send(f'{message.author.mention}, here is how {user_match(tokens[1])} and {user_match(tokens[2])} match up:\n{tokens[1]}           ----            {tokens[2]}\n-------------------------------------------------------{data}')
                return
