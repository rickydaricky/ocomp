    ...
    async def on_message(self, message):
        ...
        if tokens[0].startswith('.compare'):
            ...
            try:
                ...
                await message.channel.send(f'{message.author.mention}, here is how {Database.user_match(self, tokens[1])} and {Database.user_match(self, tokens[2])} match up:\n{Database.user_match(self, tokens[1])}           ----            {Database.user_match(self, tokens[2])}\n-------------------------------------------------------{data}')
                return
