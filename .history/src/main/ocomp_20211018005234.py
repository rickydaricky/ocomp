...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if tokens[0].startswith('.addme'):
            ...
            nonactivity = await db.set_user(tokens[1], replace_last_char(str(message.author), '#', '-'))
            if nonactivity == 0:
        ...
        if tokens[0].startswith('.compare'):
            ...
            try:
                ...
                await message.channel.send(f'{message.author.mention}, here is how '
                                           ...
                                           f'and {await Database.user_match(self, tokens[2])}'
                                           ' match up:\n'
                                           f'{await Database.user_match(self, tokens[1])}'
                                           f'           ----            '
        ...
        if tokens[0].startswith('.hero'):
            data = await db.find_hero(replace_last_char(str(message.author), '#', '-'), tokens[1].lower())
            if data == 'error':
                return
