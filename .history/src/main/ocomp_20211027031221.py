...
from database import Database, replace_last_char, search_prefix, replace_prefix, leave_guild, word_match, hero_list
...
async def single_hero_stats(ctx, hero):
    ...
    if data == 'error':
        return
    await ctx.send(f'{ctx.message.author.mention}, '
                   f'here are your stats on {word_match(hero, hero_list)} per 10: {data}')
    return
