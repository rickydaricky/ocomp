...
class Database():
    ...
    async def find_hero(self, nickname, hero):
        ...
        results = await Stats.crawl_search_hero(battle_id, hero)
        return results
...
class Stats():
    ...
    async def crawl_search_hero(battle_id, hero):
        ...
        hero_response = soup.findAll(class_ = 'theme-hero-' + hero)[0]
        print('Hero Response: ' + hero_response)
        padded_stats = hero_response.select('div.stats.padded')
        print('Padded: ' + padded_stats)
        for stat in padded_stats:
            children = stat.contents
            responses.put(children[2].string, children[0].string)
