    ...
    async def compare(self, player_1, player_2, hero):
        ...
        try:
            for key, val in Database.dict_zip(p1_data, p2_data).items():
                val_1 = val[0]
                val_2 = val[1]
                space_gap = ' ' * max(0, round(1.8 * (25 - len(val_1) - len(key))))
                if (key != 'Deaths' and val_1 > val_2) or (key == 'Deaths' and val_1 < val_2) and (key != 'Record'):
                    stat_status = f'\n{key}: __**{val_1}**__{space_gap}----          {key}: {val_2}'
                elif (key != 'Deaths' and val_1 < val_2) or (key == 'Deaths' and val_1 > val_2) and (key != 'Record'):
