...
from database import Database, replace_last_char, search_prefix, \
...
@bot.command(help='Adds the battle ID to the discord user in the database\n'
             f'Usage:\n{await get_prefix}addme battle_id \nsuch as: '
             f'{await get_prefix}addme Exor#11705')
...
@ bot.command(help='Compares the stats of the two listed players from the database.\n'
              ...
              f'\n{await get_prefix}compare Exor Bosco Ana')
...
@ bot.command(name='hero', help='Shows your stats on a specific hero!\n'
              f'Usage:\n{await get_prefix}hero [hero_name], such as:'
              f'\n{await get_prefix}hero ana')
async def single_hero_stats(ctx, hero):
    """Shows the stats of the calling discord user on a specific hero"""

    data = await db.find_hero(str(ctx.guild.id), replace_last_char(
        str(ctx.message.author), '#', '-'), hero.lower())
    if data == 'error':
        return
    await ctx.send(f'{ctx.message.author.mention}, '
                   f'here are your stats on {hero_word_match(hero)} per 10: {data}')
    return
