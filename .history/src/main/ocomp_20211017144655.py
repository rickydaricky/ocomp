    ...
    async def on_message(self, message):
        ...
        if tokens[0].startswith('.compare'):
            ...
            try:
                ...
                await message.channel.send(f'{message.author.mention}, here is how {await Database.user_match(self, tokens[1])} and {await Database.user_match(self, tokens[2])} match up:\n{await Database.user_match(self, tokens[1])}           ----            {await Database.user_match(self, tokens[2])}\n-------------------------------------------------------{data}')
                return
