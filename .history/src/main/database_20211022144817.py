    ...
    async def compare(self, player_1, player_2, hero):
        ...
        try:
            for key, val in Database.dict_zip(p1_data, p2_data).items():
                ...
                status = 0  # -1 means val_1 should be emphasized, 0 means equal, 1 means val_2 should be emphasized
                ...
                if key == 'Deaths':
                    ...
                    if val_1_unit == val_2_unit:
                        status = val_comp(val_1_unit, val_2_unit)
                    else:
                        if val_1_unit == 'hours':
                            status = -1
                    ...
                    val_1 = int(val_1.replace(',', ''))
                    val_2 = int(val_2.replace(',', ''))
                ...
                if status == 0:
                    stat_status = f'\n{key}: {val_1}{space_gap}----{ten_space}{key}: {val_2}'
                elif status == -1:
                    stat_status = f'\n{key}: __**{val_1}**__{space_gap}----{ten_space}{key}: {val_2}'
                elif status == 1:
                    stat_status = f'\n{key}: {val_1}{space_gap}----{ten_space}{key}: __**{val_2}**__'
...
def val_comp(val_1: int, val_2: int)
