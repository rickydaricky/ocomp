...
async def crawl_search_hero(battle_id, hero):
    ...
    soup = BeautifulSoup(    header_request = requests.get(overbuff_url, headers=try_header)
.content, 'html5lib')
...
async def not_active(battle_id):
    ...
    """
    overbuff_url = id_to_url(battle_id)
    header = requests.get(overbuff_url, headers=try_header)
    soup = BeautifulSoup(header.content, 'html5lib')
